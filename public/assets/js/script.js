    ...
    document.getElementById("add-module")?.addEventListener("click", () => {
        const moduleHtml = `
            <div class="module-card mb-3">
                <input type="text" name="modules[${moduleIndex}][title]" class="form-control mb-2" placeholder="Nome do Módulo">
                <textarea name="modules[${moduleIndex}][description]" class="form-control mb-2" placeholder="Descrição do Módulo"></textarea>
                <div class="lessons-container"></div>
                <button type="button" class="btn btn-sm btn-primary add-lesson" data-module="${moduleIndex}">+ Adicionar Aula</button>
            </div>
        `;
        ...
        if (e.target.classList.contains("add-lesson")) {
            ...
            const lessonHtml = `
                <div class="lesson-item mb-2">
                    <input type="text" name="modules[${moduleId}][lessons][${lessonCount}][title]" class="form-control mb-1" placeholder="Título da Aula">
                    <select name="modules[${moduleId}][lessons][${lessonCount}][type]" class="form-select mb-1">
                        <option value="video">Vídeo</option>
                        <option value="text">Texto</option>
                        <option value="quiz">Quiz</option>
                        <option value="exercise">Exercício</option>
                    </select>
                    <input type="number" name="modules[${moduleId}][lessons][${lessonCount}][duration]" class="form-control mb-1" placeholder="Duração (min)">
                    <input type="url" name="modules[${moduleId}][lessons][${lessonCount}][video_url]" class="form-control" placeholder="Link do vídeo">
                </div>
            `;
...
document.addEventListener("DOMContentLoaded", () => {
    function updatePreview() {
        const title = document.getElementById("title_course")?.value || "Título do Curso";
        const subtitle = document.getElementById("courseSubtitle")?.value || "Subtítulo do curso";
        const description = document.getElementById("courseDescription")?.value || "Descrição do curso aparecerá aqui...";

        document.getElementById("preview-title") && (document.getElementById("preview-title").textContent = title);
        document.getElementById("preview-subtitle") && (document.getElementById("preview-subtitle").textContent = subtitle);
        document.getElementById("preview-description") && (document.getElementById("preview-description").innerHTML = description);
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    publishBtn?.addEventListener("click", () => sendCourseData(form.action));

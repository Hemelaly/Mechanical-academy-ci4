...
<?= $this->section('title') ?>Dashboard - Instructores<?= $this->endSection() ?>
...
<style>
  .mx-card{background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.05));border:1px solid rgba(255,255,255,.12);border-radius:18px;}
  .mx-kpi{transition:transform .15s ease, box-shadow .15s ease;}
  .mx-kpi:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(0,0,0,.25);}
  .table > :not(caption) > * > *{vertical-align: middle;}
  .mx-avatar{width:44px;height:44px;object-fit:cover;border-radius:50%;}
  .mx-chip{border:1px solid rgba(255,255,255,.2);background:rgba(255,255,255,.06);border-radius:999px;padding:.15rem .55rem;}
  .mx-toolbar{gap:.5rem;flex-wrap:wrap;}
...
<header class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
  <div>
    <h1 class="h3 mb-1">Instrutores</h1>
    <p class="text-muted mb-0">Gerencie seus instrutores, status e progresso deles.</p>
  </div>
  <div class="d-flex mx-toolbar">
    <div class="dropdown">
      ...
      <ul class="dropdown-menu dropdown-menu-end">
        <li><a class="dropdown-item" href="<?= route_to('admin.instructors.import') ?>">Importar CSV</a></li>
        <li><a class="dropdown-item" href="<?= route_to('admin.instructors.export') ?>">Exportar CSV</a></li>
      </ul>
    </div>
    <a href="<?= route_to('admin.instructors.new') ?>" class="btn btn-primary">
      <i class="bi bi-person-plus me-2" aria-hidden="true"></i>Novo Instrutor
    </a>
  </div>
</header>
...
<div class="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
  ...
  <div class="btn-group" role="group" aria-label="Export">
    <a class="btn btn-outline-secondary btn-sm" href="<?= route_to('admin.instructors.export') ?>">
...
<section class="mx-card p-0 overflow-hidden">
  <?php if(empty($instructors)): ?>
    <div class="p-4 text-center text-muted">
      ...
      <a href="<?= route_to('admin.instructors.new') ?>" class="btn btn-primary btn-sm">Adicionar estudante</a>
    </div>
  <?php else: ?>
            ...
            <tr>
              ...
              <td>
                <div class="d-flex align-items-center">
                  ...
                  <div>
                    <a href="<?= route_to('admin.instructors.show', $s['id']) ?>" class="fw-semibold text-decoration-none"><?= esc($s['nome']) ?></a>
              ...
              <td class="text-end">
                <div class="btn-group btn-group-sm">
                  <a href="<?= route_to('admin.instructors.show', $s['id']) ?>" class="btn btn-outline-secondary" title="Ver">
                    <i class="bi bi-eye"></i>
                  </a>
                  <a href="<?= route_to('admin.instructors.edit', $s['id']) ?>" class="btn btn-outline-secondary" title="Editar">
    ...
    <div class="p-3 border-top d-flex justify-content-between align-items-center">
      <small class="text-muted">Mostrando <?= count($instructors) ?> de <?= number_to_amount($metrics['total'] ?? count($instructors), 0, 'pt_BR') ?> Instrutores</small>
      <div><?= $pagerHtml ?? '' ?></div>
    </div>
  <?php endif; ?>
</section>
...
<div class="modal fade" id="modalDelete" tabindex="-1" aria-labelledby="modalDeleteLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="<?= route_to('admin.instructors.delete') ?>" class="modal-content">
      <?= csrf_field() ?>
...
<div class="modal fade" id="modalEmail" tabindex="-1" aria-labelledby="modalEmailLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="<?= route_to('admin.instructors.bulkEmail') ?>" class="modal-content">
      <?= csrf_field() ?>
      <div class="modal-header">
...
<script>
  ...
  checkAll?.addEventListener('change',()=>{ rowChecks.forEach(c=> c.checked = checkAll.checked); updateBulk(); });
  ...
  document.getElementById('bulkSuspend') ?.addEventListener('click',()=>postBulk('<?= route_to('admin.instructors.bulkSuspend') ?>'));
  document.getElementById('bulkActivate')?.addEventListener('click',()=>postBulk('<?= route_to('admin.instructors.bulkActivate') ?>'));
  document.getElementById('bulkDelete')  ?.addEventListener('click',()=>postBulk('<?= route_to('admin.instructors.bulkDelete') ?>'));
</script>

...
<?= $this->section('title') ?>Matrículas e vendas<?= $this->endSection() ?>

<?= $this->section('students') ?>
<div class="min-w-0 space-y-6">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">Matrículas e vendas</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Pagamentos pendentes, quem comprou, progresso e controlo de acesso (toda a plataforma).</p>
        </div>
...
<script>
    (function () {
        const enrollEndpoint = <?= json_encode(site_url('admin/dashboard/matriculas/data')) ?>;
        const pendingEndpoint = <?= json_encode(site_url('admin/dashboard/matriculas/pending')) ?>;
        const toggleEndpointBase = <?= json_encode(site_url('admin/dashboard/matriculas/toggle')) ?>;
        const approveBase = <?= json_encode(site_url('admin/dashboard/matriculas')) ?>;
        const manualEnrollEndpoint = <?= json_encode(site_url('admin/dashboard/matriculas/matricular')) ?>;
        const demoEndpoint = <?= json_encode(site_url('admin/dashboard/matriculas/demo')) ?>;
        ...
        const loadEnrollments = () => {
            ...
            fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                ...
                .then(data => {
                    ...
                    if (!items.length) {
                        enrollBody.innerHTML = '<tr><td colspan="8" class="px-6 py-6 text-center text-slate-500">Nenhum aluno encontrado.</td></tr>';
                    } else {
                        enrollBody.innerHTML = items.map(item => {
                            ...
                            return `
                                <tr class="border-b border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
                                    ...
                                    <td class="px-6 py-4">${escapeHtml(item.instructor_name || '—')}</td>
                                    <td class="px-6 py-4">
